...
def generate_invoice_number_new(conn):
    """Generate invoice number: S2627-1"""
    try:
        cursor = conn.cursor(dictionary=True)
        now = datetime.now()
        if now.month >= 4:
        ...
        cursor.execute("SELECT * FROM pos_invoice_numbers WHERE financial_year = %s", (fy,))
        rec = cursor.fetchone()
        if rec:
            seq = rec['sequence'] + 1
            cursor.execute("UPDATE pos_invoice_numbers SET sequence=%s WHERE financial_year=%s", (seq, fy))
        else:
            seq = 1
            cursor.execute("INSERT INTO pos_invoice_numbers (financial_year, sequence) VALUES (%s,%s)", (fy, seq))
        conn.commit()
        return f"S{fy_short}-{seq}"
    except Exception as e:
        print(f"Error generating invoice number: {str(e)}")
...
def generate_invoice_pdf(order_id, conn, app, invoice_number=None):
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM pos_orders o WHERE o.id=%s
        """, (order_id,))
        ...
        if not order.get('invoice_number'):
            cursor.execute("UPDATE pos_orders SET invoice_number=%s, invoice_date=NOW() WHERE id=%s",
        ...
        cursor.execute("""
            ...
            FROM pos_order_items oi LEFT JOIN products p ON oi.product_id=p.id
            WHERE oi.order_id=%s
        """, (order_id,))
        ...
        def P(txt, s): return Paragraph(str(txt), sty[s])
        ...
        if order.get('customer_name'):
            customer_lines.append(f"<b>{order.get('customer_name')}</b>")
        if order.get('customer_mobile'):
            customer_lines.append(f"Phone: {order.get('customer_mobile')}")
        if order.get('customer_email'):
            customer_lines.append(f"Email: {order.get('customer_email')}")
            
        customer_address = "<br/>".join(customer_lines) if customer_lines else "Walk-in Customer"
        ...
        if payment_method == 'SPLIT':
            split_data = json.loads(order.get('split_payments') or '[]')
            split_lines = [f"{s.get('method')}: ₹{s.get('amount')}" for s in split_data]
            payment_info = f"Mode of payment: SPLIT<br/>" + "<br/>".join(split_lines)
        else:
...
def calculate_gst_breakdown(order_id, conn):
    """Calculate GST breakdown for a POS order"""
    try:
        cursor = conn.cursor(dictionary=True)
        ...
        cursor.execute("SELECT * FROM pos_orders WHERE id=%s", (order_id,))
        order = cursor.fetchone()
        if not order:
        ...
        if order.get('taxable_amount') is not None and float(order.get('taxable_amount')) > 0:
            ...
            for k in breakdown:
                if isinstance(breakdown[k], Decimal):
                    breakdown[k] = float(breakdown[k])
            return breakdown

        cursor.execute("""
            SELECT oi.*, p.gst_rate, p.gst_rate as tax_value, p.hsn_code,
                   COALESCE(oi.price, 0) as unit_price
            FROM pos_order_items oi JOIN products p ON oi.product_id=p.id
            WHERE oi.order_id=%s
        """, (order_id,))
        items = cursor.fetchall()
